 ...
 * 代价是**一个深色值只能翻成一个浅色值**：同一个深色值在界面上担着两个角色、
 * 两个角色又想要不同浅色时，键上可以带 "@角色" 后缀分开 ——
 * `Theme.c("#1e1f22@tabStrip", Theme.rev)` 先查带角色的那条，查不到再按裸 hex 查，
 * 都查不到返回去掉后缀的 hex 本身（所以深色档仍然恒等）。方案文件里同样的写法。
 *
 ...
 * 那段注释）。函数体里读 m_light，绑定是醒不过来的。把当前方案的那个号当参数传进去
 * （现在传 Theme.rev，见下面那条），属性读取发生在绑定表达式里，换方案时才会重算。
