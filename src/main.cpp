 ...
 *
 * 背景：这些 QtWidgets 弹框（截图失败 / 贴图失败时那几个 QMessageBox）弹出时，
...
int main(int argc, char *argv[]) {
     ...
     *
     * 下面这几组角色管的是还留在 QtWidgets 那侧的东西：截图 / 贴图失败时的
     * QMessageBox。不设的话它是系统浅色，深色界面里点一下弹出一块白。
     *
     * 注意：提示 / 确认 / 未保存改动、还有"要用户敲字"那三类（重命名 / 转到行 /
     * 参考线列）**都不再**依赖这里 —— 它们已经换成
    ...
    if (SelfTest::enabled(argc, argv)) {
         ...
         *
         * 这个数**跟着套件的长度走**：2026-09-24 加了配色方案 / 字体那批判据之后，
         * 实测整跑一遍要 62 秒（原来 60 秒的闸刚好把它掐了 —— 被掐那几次红的是
         * 后半段随机几条，看着像功能坏了，其实是跑到一半就没跑完）。
         * 现在留 ~60% 余量；再加判据请顺手重测一次时长。
         */
        QTimer::singleShot(100000, &app, []() {
            qWarning("自检超时，强制退出");
            ::exit(9);
        });
